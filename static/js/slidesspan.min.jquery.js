		...
		return this.each(function() {
			...
			if (option.generatePagination) {
				...
				control.children().each(function() {
					$('.' + option.paginationClass, elem).append('<li href="#' + number + '">' + (number + 1) + '</li>');
					number++;
				});
			} else {
			...
			$('.' + option.paginationClass + ' li[href=#' + start + ']', elem).addClass('current');
			$('.' + option.paginationClass + ' li', elem).mousemove(function() {
				if (option.play) {
					pause();
				};
				...
				if (!active && loaded) {
					...
					switch (direction) {
						...
						case 'pagination':
							...
							prev = $('.' + option.paginationClass + ' li.current', elem).attr('href').replace('#', '');
							if (next > prev) {
								position = width * 2;
								direction = -width * 2;
							} else {
					...
					if (option.pagination) {
						...
						$('.' + option.paginationClass + ' li[href=#' + next + ']', elem).addClass('current');
					}
				}
			};
		});
